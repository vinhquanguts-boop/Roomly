...
export function HowItWorks() {
  return (
    <section id="how-it-works" className="bg-bg-elevated px-5 py-14 md:px-8 md:py-[72px]">
      <div className="mx-auto max-w-[1120px] text-center">
        ...
        <div className="relative mt-8 grid gap-8 md:grid-cols-4 md:gap-6">
          <div
          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.title} className="relative flex flex-col items-center" data-reveal>
                <span className="relative z-10 flex size-[70px] items-center justify-center rounded-full border border-roomly-secondary bg-bg-elevated text-accent shadow-[0_4px_18px_rgba(26,22,20,0.08)] md:size-[74px]">
                  <Icon className="size-8 stroke-[1.8]" aria-hidden="true" />
                </span>
                <h3 className="mt-4 text-[15px] font-bold text-text-primary">{step.title}</h3>
                <p className="mt-2 max-w-[190px] text-sm leading-5 text-text-secondary">{step.description}</p>
              </div>
            );
          })}
        </div>
