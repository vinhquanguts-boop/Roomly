...
export function GeneratingPage() {
  ...
  }, [design?.id, design?.status, navigate]);
      ...
      <LightCard className="mx-auto max-w-[640px] p-6 md:p-8">
        <span className="mx-auto flex size-14 items-center justify-center rounded-full bg-secondary-muted text-accent">
          {status === 'failed' || hasLoadError ? (
            ...
            <Sparkles
              className="size-7 animate-[spin-slow_3s_linear_infinite]"
              aria-hidden="true"
            />
          )}
        </span>
        <p className="mt-5 text-center text-xs font-bold uppercase tracking-[0.24em] text-accent">Step 4</p>
                  ...
                  <div key={step.status} className="flex items-center gap-4">
                    ...
                    <span
                      className={[
                        ...
                        done || active || designQuery.isPending ? 'text-text-primary' : 'text-text-secondary',
                      ].join(' ')}
                    >
                      {step.label}
                    </span>
